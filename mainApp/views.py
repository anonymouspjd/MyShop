...
def buyerProfile(request):
    ...
    check = Checkout.objects.filter(buyer=buyer)
    return render(request,"buyerProfile.html",{"User":buyer,"Checkout":check,"Wishlist":wishlist})
...
def forgetPassword(request):
    if (request.method=="POST"):
        ...
        user.save()
        subject = '''OTP of forget password'''
        message = ''' 
                       Hello!!!
                       Team : Myshop.com
                       OTP : %d
        ...
        messages.success(request,"OTP is sent to your register email id")
...
def confirmOTP(request,username):
    if(request.method=="POST"):
        ...
        try:
            ...
            user = Buyer.objects.get(username=username)
        if (user.otp==otp):
            return HttpResponseRedirect("/newpassword/"+username+"/")
        else:
